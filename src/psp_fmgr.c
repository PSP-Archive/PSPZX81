...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    if ((!strcasecmp(pszExt, ".p")) || 
        (!strcasecmp(pszExt, ".81"))) {
      return FMGR_FORMAT_PRG;
...
static char user_filedir_prg[PSP_FMGR_MAX_NAME];
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      if (file_format == FMGR_FORMAT_ZIP) {
        ...
        if (user_file_format == FMGR_FORMAT_PRG) /* Prg */ error = zx81_load_program(user_filename, 1);
      }
      else 
      {
        if (file_format == FMGR_FORMAT_PRG) /* Prg */ error = zx81_load_program(user_filename, 0);
        else
        if (file_format == FMGR_FORMAT_KBD) /* Kbd */  error = psp_kbd_load_mapping(user_filename);
        else
        if (file_format == FMGR_FORMAT_SET) /* Settings */  error = zx81_load_file_settings(user_filename);
      }
    }
