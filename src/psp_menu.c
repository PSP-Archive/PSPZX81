...
psp_menu_display_save_name()
{
  ...
  snprintf(buffer, 20, "Game: %s", ZX81.zx81_save_name);
...
psp_display_screen_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_SCREENSHOT) {
      sprintf(buffer,"%d", ZX81.psp_screenshot_id);
  ...
  for (slot_id = 0; slot_id < ZX81_MAX_SAVE_STATE; slot_id++) {
    if (slot_id == cur_slot) color = PSP_MENU_SEL2_COLOR;
    else                     color = PSP_MENU_TEXT_COLOR;

    if (ZX81.zx81_save_state[slot_id].used) {
      ScePspDateTime* my_date = &ZX81.zx81_save_state[slot_id].date;
# ifndef LINUX_MODE
...
# endif
  ...
  if (ZX81.zx81_save_state[cur_slot].thumb) {
    psp_sdl_blit_thumb(310, 40, ZX81.zx81_save_state[cur_slot].surface);
  } else {
    psp_sdl_blit_thumb(310, 40, thumb_surface);
  }
...
psp_main_menu_reset(void)
{
  ...
  psp_sdl_back2_print(140, 160, "Reset ZX81 !", PSP_MENU_WARNING_COLOR);
  psp_menu_dirty = 1;
  psp_sdl_flip();
  emulator_reset();
  reset_save_name();
...
{
  if (ZX81.zx81_save_state[cur_slot].used) {
    return zx81_snapshot_load_slot(cur_slot);
  }
  return -1;
}
...
{
  return zx81_snapshot_save_slot(cur_slot);
}
...
psp_main_menu_load_slot()
{
  ...
  error = zx81_snapshot_load_slot(cur_slot);
...
{
  int error;
  error = zx81_snapshot_save_slot(cur_slot);
...
psp_main_menu_del_slot()
{
  ...
  error = zx81_snapshot_del_slot(cur_slot);
...
{
  if (step == 1) {
    cur_slot++; if (cur_slot >= ZX81_MAX_SAVE_STATE) cur_slot = 0;
  } else if (step == -1) {
    cur_slot--; if (cur_slot < 0) cur_slot = ZX81_MAX_SAVE_STATE - 1;
  }
}
        ...
        (new_pad & PSP_CTRL_LEFT  )) 
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        case MENU_LOAD_PRG  : psp_menu_dirty = 1;
                              if (psp_main_menu_load(FMGR_FORMAT_PRG)) {
                                end_menu = 1;
                              }
                              psp_menu_dirty = 1;
                              old_pad = new_pad = 0;
