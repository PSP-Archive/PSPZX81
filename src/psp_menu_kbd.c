...
psp_display_screen_kbd_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_KBD_ITEM; menu_id++) 
  {
        ...
        (menu_id <= MENU_KBD_JOY_RIGHT)) 
    {
      ...
      if (menu_kbd_selected == KBD_NORMAL_MAPPING  ) zx81_key = loc_kbd_mapping[kbd_id];
      else
      if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) zx81_key = loc_kbd_mapping_L[kbd_id];
      else
      if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) zx81_key = loc_kbd_mapping_R[kbd_id];

      if ((zx81_key >= 0) && (zx81_key < ZX81_MAX_KEY)) {
        strcpy(buffer, psp_zx81_key_to_name[zx81_key].name);
      } else 
      if (zx81_key == KBD_UNASSIGNED) {
        sprintf(buffer, "UNASSIGNED");
      } else
      if (zx81_key == KBD_LTRIGGER_MAPPING) {
        sprintf(buffer, "L MAPPING");
      } else
      if (zx81_key == KBD_RTRIGGER_MAPPING) {
        sprintf(buffer, "R MAPPING");
      } else {
        sprintf(buffer, "KEY %d", zx81_key);
      }
...
psp_keyboard_menu_mapping(int kbd_id, int step)
{
  ...
  if (ploc_kbd_mapping[kbd_id] <  -3) ploc_kbd_mapping[kbd_id] = ZX81_MAX_KEY-1;
  else
  if (ploc_kbd_mapping[kbd_id] >= ZX81_MAX_KEY) ploc_kbd_mapping[kbd_id] = -3;
}
    ...
    if (psp_irkeyb != PSP_IRKEYB_EMPTY) {
      ...
      if (zx81_key != -1) {
            ...
            (cur_menu_id <= MENU_KBD_JOY_RIGHT)) 
        {
          ...
          if (menu_kbd_selected == -1) loc_kbd_mapping[kbd_id] = zx81_key;
          else
          if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) loc_kbd_mapping_L[kbd_id] = zx81_key;
          else
          if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) loc_kbd_mapping_R[kbd_id] = zx81_key;
        }
      }
    }
# endif
